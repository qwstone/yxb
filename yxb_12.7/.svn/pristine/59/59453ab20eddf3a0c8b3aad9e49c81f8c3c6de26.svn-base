...
@interface MessageCenterController ()<HeaderViewDelegate>
{
    NSInteger tag;
...
-(void)dealloc{
    NSLog(@"MessageCenterController is dealloc");
}
...
- (void)viewDidLoad {
    ...
    //注册 未读消息全部标记为已读 通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(MessageAllRead:) name:@"MessageAllRead" object:nil];
...
- (void)_createUI{
    ...
    UIImage *rightImg = [UIImage imageNamed:@"ReadAllMsg"];
    UIButton * SetAllReadBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    SetAllReadBtn.frame = CGRectMake(0, 0, rightImg.size.width, rightImg.size.height);
    [SetAllReadBtn addTarget:self action:@selector(ReadAllMsgAction:) forControlEvents:UIControlEventTouchUpInside];
    [SetAllReadBtn setImage:[UIImage imageNamed:@"ReadAllMsg"] forState:UIControlStateNormal];
    UIBarButtonItem * navItem = [[UIBarButtonItem alloc]initWithCustomView:SetAllReadBtn];
    ...
    _tableView = [[QCBaseTableView alloc]initWithFrame:CGRectMake(0, _headerView.bottom+5, kDeviceWidth, kDeviceHeight-64-48-49) style:UITableViewStylePlain refreshFooter:YES];
...
#pragma mark ----------- 全部标记为已读
//czg
- (void)ReadAllMsgAction:(UIButton*)sender
{
    [[SetMessageReadManager sharedInstance]setAllMessageRead];
}

-(void)MessageAllRead:(NSNotification*)info
{
    NSLog(@"*********************开始刷新");
    page = MessageCenterPageNOStart;
...
- (void)httpDowloadWithListStyle
{
    ...
    __weak MessageCenterController *this = self;
...
-(void)httpLoadComplete:(NSSkyArray *)r{
    if (r.errCode==0) {
        ...
        NSLog(@"ttt%lutttt",(unsigned long)r.iArray.count);
        if (r.iArray.count != 0 && r.iArray.count%20 == 0) {
            self.tableView.hasFooter = YES;
        }
        else if ( r.iArray.count%20 >0) {
            self.tableView.hasFooter = YES;
        }else {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (h > 30 && !self.tableView.isRefresh && self.tableView.hasFooter == YES) {
        if (_dataArray.count%20==0||_dataArray.count%20>0) {
            page ++;
            [self httpDowloadWithListStyle];
        }else {
