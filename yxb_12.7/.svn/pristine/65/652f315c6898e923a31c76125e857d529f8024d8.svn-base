...
//        [currentWindow addGestureRecognizer:tap];
...
- (id)init
{
    ...
    if (self) {
        ...
        //点击空白地方 消失
        self.tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(closeWithGesture:)];
        UIWindow *currentWindow = [[[UIApplication sharedApplication] windows] firstObject];
        _tap.delegate = self;
        [currentWindow addGestureRecognizer:_tap];
...
- (void)close
{
    ...
    if (_tap) {
        [_tap removeTarget:self action:@selector(closeWithGesture:)];
        UIWindow *currentWindow = [[[UIApplication sharedApplication] windows] firstObject];
        [currentWindow removeGestureRecognizer:_tap];

    }
}

-(void)closeWithGesture:(UITapGestureRecognizer *)tap
{
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    CGRect frame = CGRectMake(self.frame.origin.x, subView.frame.origin.y, self.width, containerView.height);
    bool isIn = CGRectContainsPoint(frame, p);
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    CGFloat buttonWidth = (container.bounds.size.width-[buttonTitles count]+1) / [buttonTitles count];
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        else if (i == ([buttonTitles count]-1) && [buttonTitles count] > 1) {
        ...
        if ((i == 0) || (i == [buttonTitles count] - 1)) {
            UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:closeButton.bounds byRoundingCorners:corner cornerRadii:CGSizeMake(10, 10)];
        ...
        {
            UIView *yLine = [[UIView alloc] initWithFrame:CGRectMake(i * buttonWidth+buttonWidth, container.bounds.size.height - buttonHeight, 1, buttonHeight)];
            yLine.backgroundColor = [UIColor lightGrayColor];
            [container addSubview:yLine];
        }
    }
    
    if (containerView) {
        containerView.userInteractionEnabled = YES;
        [self setAlertContent:containerView];
    }
