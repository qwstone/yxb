...
//    //    _lastUpdateTimeLabel.textColor = rgb(255, 80, 0, 1);
//    // 3.状态标签
//    [self addSubview:_statusLabel = [self labelWithFontSize:13]];
//    //    _statusLabel.textColor = rgb(255, 80, 0, 1);
    
    /*
     change by 彭诚
     */
    CGFloat containsW = 180;
    
    UIView *contains = [[UIView alloc] initWithFrame:CGRectMake(0, 0, containsW, kViewHeight)];
    contains.backgroundColor = [UIColor clearColor];
    [self addSubview:contains];
    
    contains.center = CGPointMake(kDeviceWidth/2.0, contains.center.y);
    
    
    // 2.时间标签
    [contains addSubview:_lastUpdateTimeLabel = [self labelWithFontSize:12]];
    
    // 3.状态标签
    [contains addSubview:_statusLabel = [self labelWithFontSize:13]];
    /*
     change end
     */
    ...
    */
    NSArray *animationArray=[NSArray arrayWithObjects:
                             [UIImage imageNamed:@"yxb_refresh_anim1.png"],
                             [UIImage imageNamed:@"yxb_refresh_anim2.png"],
                             nil];
...
//    animationView.backgroundColor=[UIColor purpleColor];
    ...
    [contains addSubview:_animationView];
    ...
    _statusLabel.center = CGPointMake(kDeviceWidth/2.0, _statusLabel.center.y);
    _lastUpdateTimeLabel.center = CGPointMake(kDeviceWidth/2.0, _lastUpdateTimeLabel.center.y);
    _animationView.right = _lastUpdateTimeLabel.left;
}
...
//    _animationView.frame = CGRectMake(0, statusY, 40, 41);
//    _animationView.center = CGPointMake(_arrowImage.center.x, _animationView.center.y);
    /*
     位置修改
     change by 彭诚
     */
    CGFloat statusY = 5;
    CGFloat w = 180.0;
    CGFloat pictureW = 40.0;
    
    // 1.状态标签
    CGFloat statusX = pictureW+20;
    CGFloat statusHeight = 20;
    CGFloat statusWidth = w;
    _statusLabel.frame = CGRectMake(statusX, statusY, statusWidth, statusHeight);
    ...
    // 3.箭头
    CGFloat arrowX = pictureW/2;
    _arrowImage.center = CGPointMake(arrowX, frame.size.height * 0.5);
    
    // 4.指示器
    //    _activityView.center = _arrowImage.center;
    _animationView.frame = CGRectMake(0, statusY, pictureW, 41);
    _animationView.center = CGPointMake(_arrowImage.center.x, _animationView.center.y);
}
