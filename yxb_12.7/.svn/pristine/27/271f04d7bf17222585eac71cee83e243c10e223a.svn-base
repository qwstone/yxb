...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
	...
	if (self) {
        ...
        self.cellWidth = (kDeviceWidth - 5 *ELCAssetCellBetweenSpace)/4.0;
	}
	return self;
}
...
- (void)cellTapped:(UITapGestureRecognizer *)tapRecognizer
{
    ...
    CGFloat totalWidth = c * _cellWidth + (c - 1) * ELCAssetCellBetweenSpace;
    CGFloat startX;
    
    if (self.alignmentLeft) {
        startX = ELCAssetCellBetweenSpace;
    }else {
        startX = (self.bounds.size.width - totalWidth) / 2;
    }
    
	CGRect frame = CGRectMake(startX, 2, _cellWidth, _cellWidth);
	...
	for (int i = 0; i < [_rowAssets count]; ++i) {
        if (CGRectContainsPoint(frame, point)) {
            ...
            overlayView.width = _cellWidth;
            overlayView.height = _cellWidth;
            if (asset.selected) {
        ...
        frame.origin.x = frame.origin.x + frame.size.width + ELCAssetCellBetweenSpace;
    }
}
...
- (void)layoutSubviews
{
	...
	CGRect frame = CGRectMake(startX, ELCAssetCellBetweenSpace/2, _cellWidth, _cellWidth);
