...
- (ZXQRCodeFinderPattern *)combineEstimateI:(float)i j:(float)j newModuleSize:(float)newModuleSize {
  int combinedCount = self.count1 + 1;
  float combinedX = (self.count1 * self.x + j) / combinedCount;
  float combinedY = (self.count1 * self.y + i) / combinedCount;
  float combinedModuleSize = (self.count1 * self.estimatedModuleSize + newModuleSize) / combinedCount;
