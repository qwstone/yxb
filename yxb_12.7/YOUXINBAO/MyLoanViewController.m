...
//    moneyLabel.text=self.loanInforDetailsModel.money;
//    NSString*lixi= self.loanInforDetailsModel.interest;
//    NSString*benjin=self.loanInforDetailsModel.money;
    //    double benxistr=[lixi doubleValue]+[benjin doubleValue];

     NSString*totalNeedRepay=self.loanInforDetailsModel.totalNeedRepay;
    moneyLabel.text=[NSString stringWithFormat:@"%@",totalNeedRepay ];
     UILabel *ZongEMony=[_yanqi.whiteView viewWithTag:380002];
    NSString*BuChangMony=_yanqi.changeStr;
...
- (void)YanqiConfirmClick{
    if ([_yanqi.textFiled1.text isEqualToString:@""]||[_yanqi.Timebtn.titleLabel.text isEqualToString:@"请选择时间"]) {
        ...
        compensationMoney=_yanqi.textFiled1.text;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dialogArray.count) {
        ...
        if ([model.dialogueMode integerValue]==1  || [model.dialogueMode integerValue] == 5) {//文字
            ...
            return 168.0/2.0+10;
        }else if ([model.dialogueMode integerValue]==3){//申请延期
            return 180;
        }
        else{
            return 120;
...
- (void)httpLOadParams:(NSString *)s httpOperation:(HttpOperator *)httpOperation{
    LoanManagerV9 *manager = (LoanManagerV9 *)[httpOperation getAopInstance:[LoanManagerV9 class] returnValue:[YXBLoanInfoDetails class]];
    QCUserModel * userModel = [[QCUserManager sharedInstance]getLoginUser];
...
-(void)httpSendMesLOadParams:(NSString *)s httpOperation:(HttpOperator *)httpOperation ChatType:(NSInteger)chatType Content:(NSString *)content{
    LoanManagerV9 *_activityM = (LoanManagerV9 *)[httpOperation getAopInstance:[LoanManagerV9 class] returnValue:[TResultSet class]];
...
-(void)httpClickedBottomBtnLOadParams:(NSString *)s httpOperation:(HttpOperator *)httpOperation bottomBtnId:(NSInteger)btnId{
    LoanManagerV9 *_activityM = (LoanManagerV9 *)[httpOperation getAopInstance:[LoanManagerV9 class] returnValue:[YXBLoanInfoDetails class]];
