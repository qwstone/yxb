...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
	if (self = [super initWithCoder:aDecoder]) {
		...
		// Appearance
		_backgroundFillColor = [UIColor colorWithWhite:0.0f alpha:0.0f];
		_backgroundTintColor = [UIColor whiteColor];
		
		_circleStrokeForegroundColor = [UIColor darkGrayColor];
		_circleStrokeBackgroundColor = [_circleStrokeForegroundColor colorWithAlphaComponent:0.0f];
				...
				view:(UIView *)superview
{
	...
	if (self.style != KVNProgressStyleProgress) {
		...
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC/2)), dispatch_get_main_queue(), ^{
...
- (void)animateAppearance
{
	...
	self.alpha = 0.0f;
	self.contentView.transform = CGAffineTransformScale(self.contentView.transform, 1.0f, 1.0f);
