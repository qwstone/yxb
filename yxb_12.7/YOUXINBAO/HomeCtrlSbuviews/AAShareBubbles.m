...
-(void)show
{
    if(!self.isAnimating)
    {
        ...
        // Create background
        bgView = [[UIView alloc] initWithFrame:mWindow.bounds];
        ...
        UITapGestureRecognizer *tapgess = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(shareViewBackgroundTapped:)];
        [bgView addGestureRecognizer:tapgess];
        [self addGestureRecognizer:tapges];
        [self.parentView addSubview:bgView];
        [self.parentView insertSubview:bgView belowSubview:self];
        // --
        ...
        if(self.showFacebookBubble) {
            UIButton *facebookBubble = [self shareButtonWithIcon:@"footimg-01" andBackgroundColorRGB:self.facebookBackgroundColorRGB];
        ...
        if(self.showTwitterBubble) {
            UIButton *twitterBubble = [self shareButtonWithIcon:@"footimg-02" andBackgroundColorRGB:self.twitterBackgroundColorRGB];
        ...
        if(self.showGooglePlusBubble) {
            UIButton *googlePlusBubble = [self shareButtonWithIcon:@"footimg-03" andBackgroundColorRGB:self.googlePlusBackgroundColorRGB];
        ...
        if(self.showTumblrBubble) {
            UIButton *googlePlusBubble = [self shareButtonWithIcon:@"footimg-04" andBackgroundColorRGB:self.tumblrBackgroundColorRGB];
        ...
        if(self.showMailBubble) {
            UIButton *mailBubble = [self shareButtonWithIcon:@"footimg-05" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        ...
        float bubblesBetweenAngel = 90 / bubbles.count;
        float angely = (360 - bubblesBetweenAngel) * 0.5;
        float startAngel = 80 - angely;
...
//    circle.backgroundColor = [self colorFromRGB:rgb];
     circle.backgroundColor = [UIColor groupTableViewBackgroundColor];
...
//    button.top = (button.top - self.parentView.top);
    return button;
}
