...
- (int)findRowSkip {
  ...
  for (int i = 0; i < max; i++) {
    ...
    if ([center count1] >= ZX_CENTER_QUORUM) {
...
- (BOOL)haveMultiplyConfirmedCenters {
  ...
  for (int i = 0; i < max; i++) {
    ...
    if ([pattern count1] >= ZX_CENTER_QUORUM) {
      confirmedCount++;
...
NSInteger centerCompare(id center1, id center2, void *context) {
  ...
  if ([((ZXQRCodeFinderPattern *)center2) count1] == [((ZXQRCodeFinderPattern *)center1) count1]) {
    ...
    return [((ZXQRCodeFinderPattern *)center2) count1] - [((ZXQRCodeFinderPattern *)center1) count1];
  }
}
