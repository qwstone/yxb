...
@implementation FinancialManagerV2
/**
 * 获取全部产品
 *
 * @param userToken
...
{
    return [[AssetDetails alloc] init];
}
...
-(MyAssetListDetail *) getMyAssetListDetail:(NSString *)userToken recordId:(NSInteger)recordId {
    return [[MyAssetListDetail alloc] init];
}
/**
 * 获取理财收益
 * @param usertoken
...
-(FinanceReturn *) getFinanceReturn:(NSString *)userToken {
    return [[FinanceReturn alloc] init];
}
/**
 * 获取理财购买所需数据
 * 用户剩余金额
...
-(FinanceBuyData*)getFinanceBuyData:(NSString*)usertoken assetID:(NSString*)assetID{
    return [[FinanceBuyData alloc] init];
}
@end
